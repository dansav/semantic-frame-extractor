# From https://huggingface.co/Qwen/Qwen3-VL-Embedding-2B/blob/main/scripts/qwen3_vl_embedding.py
# Modified to add MPS (Apple Silicon) support
...
class Qwen3VLEmbedder:
    def __init__(
        ...
        **kwargs,
        ...
        if torch.cuda.is_available():
            ...
            # ROCm (AMD GPUs) may not fully support bfloat16, especially on WSL2
        ...
        print(f"Loading {model_name_or_path} on {device} with {dtype}...")

        self.model = Qwen3VLForEmbedding.from_pretrained(
            model_name_or_path, trust_remote_code=True, torch_dtype=dtype, **kwargs
        ).to(device)
    ...
    def process(
        ...
        # Move to device and cast float tensors to the right dtype
        processed_inputs = {
            k: v.to(self.model.device, dtype=self.dtype)
